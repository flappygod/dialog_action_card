...
enum DialogActionHeroFlightDirection {
...
class CustomCurveHero extends StatefulWidget {
  ...
  /// The curve applied to the hero's flight animation.
  ///
  ...
  static Map<Object, _HeroState> _allHeroesFor(
    ...
    void visitor(Element element) {
      ...
      } else if (widget is DialogActionHeroMode && !widget.enabled) {
        return;
      }
...
class _HeroFlightManifest {
  ...
  final DialogActionHeroFlightDirection type;
  ...
  Animation<double> get animation {
    return _animation ??= CurvedAnimation(
      parent: (type == DialogActionHeroFlightDirection.push)
          ? toRoute.animation!
          : fromRoute.animation!,
      curve: curve,
...
class _HeroFlight {
  ...
  static final Animatable<double> _reverseTween = Tween<double>(
    begin: 1.0,
    end: 0.0,
  );
  ...
  void start(_HeroFlightManifest initialManifest) {
    ...
    assert(() {
      ...
      final DialogActionHeroFlightDirection type = initialManifest.type;
      switch (type) {
  ...
  void divert(_HeroFlightManifest newManifest) {
    ...
    if (manifest.type == DialogActionHeroFlightDirection.push &&
        newManifest.type == DialogActionHeroFlightDirection.pop) {
    ...
    } else if (manifest.type == DialogActionHeroFlightDirection.pop &&
        newManifest.type == DialogActionHeroFlightDirection.push) {
      ...
      if (newManifest.type == DialogActionHeroFlightDirection.pop) {
        _proxyAnimation.parent = ReverseAnimation(newManifest.animation);
      } else {
...
class CustomCurveHeroController extends NavigatorObserver {
  ...
  void didStopUserGesture() {
    ...
    bool isInvalidFlight(_HeroFlight flight) {
      return flight.manifest.isUserGestureTransition &&
          flight.manifest.type == DialogActionHeroFlightDirection.pop &&
  ...
  void _maybeStartHeroTransition({
    ...
    final DialogActionHeroFlightDirection? flightType;
    switch ((
      ...
      case (_, _, AnimationStatus.forward):
        flightType = DialogActionHeroFlightDirection.push;
      default:
        flightType = null;
    }
  ...
  void _startHeroTransition(
    ...
    DialogActionHeroFlightDirection? flightType,
    bool isUserGestureTransition,
  ) {
...
extension CustomCurveHeroNavigatorObserversX<T extends NavigatorObserver>
    on List<T> {
